...
//        var y = JSON.stringify(axEventObject);
        ...
        if(!showCaseDescriptions) {
         ...
         //prototipob
        if ( (controle != 1) && (localStorage.codigo != "") && (localStorage.nome != "") && ($jobj('u7_input').val() != "") && (localStorage.terapeuta != "") ){
        
      
          $.ajax({
              type: 'POST',
              async: false,
              url: 'http://www.caed-lab.com/laiza/brasil/gamificado/api_dados.php',
              data: {
                'codigo': localStorage.codigo, // código do aluno
                'nome': localStorage.nome, //  nome do aluno
                 'n_sessao': $jobj('u7_input').val(), // numero da sessao
        ...
        var instanteAtual = Date();
          ...
          $.ajax({
            type: 'POST',
            async: true,
            url: 'http://www.caed-lab.com/laiza/brasil/gamificado/api_clique.php',
            data: {
            ...
            if(($ax.public.fn.IsTextArea(dObj.type) || $ax.public.fn.IsTextBox(dObj.type)) && hasPlaceholder) {
                ...
                inputJobj.bind('focus', function () {
                    ...
                    $ax.placeholderManager.moveCaret(this.id);
					  ...
					  //prototipob
					  localStorage.setItem('codigo', $jobj('u9_input').val());
					  localStorage.setItem('nome', $jobj('u5_input').val());
					  localStorage.setItem('n_sessao', $jobj('u7_input').val());
					  localStorage.setItem('terapeuta', $jobj('u6_input').val());
            ...
            if(message == 'setGlobalVar') {
               ...
               if (data.globalVarValue != 0){
                  $.ajax({
                    type: 'POST',
                    async: true,
                    url: 'http://www.caed-lab.com/laiza/brasil/gamificado/api_variavel.php',
                    data: {
                      'codigo': localStorage.codigo, // código do aluno
